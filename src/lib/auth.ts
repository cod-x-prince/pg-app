...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        if (!rl.success)
          throw new Error("Too many login attempts. Wait 15 minutes.");
        ...
        const dummyHash =
          "$2a$12$dummy.hash.to.prevent.timing.attack.on.user.enum";
        const isValid = user
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        // Re-fetch from DB so revoked approvals take effect immediately
        ...
        (session.user as any).isApproved =
          freshUser?.isApproved ?? token.isApproved;
      }
      return session;
    },
  },
