...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/admin/owners?kycStatus=PENDING")
        .then((r) => r.json())
        .catch(() => []),
  ...
  const toggleVerified = async (id: string, current: boolean) => {
    ...
    if (res.ok) {
      setProperties((prev) =>
        prev.map((p) => (p.id === id ? { ...p, isVerified: !current } : p)),
      );
          ...
          {tab === "kyc" &&
            (kycOwners.length === 0 ? (
              <div className="rounded-xl bg-popover shadow-soft p-12 text-center">
                ...
                <p className="font-body text-sm text-muted-foreground">
                  No owners have submitted KYC documents.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {kycOwners.map((owner: any) => (
                  <div
                    key={owner.id}
                    className="rounded-xl bg-popover shadow-soft p-5"
                  >
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="section-wrap py-8">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <a href="/admin/system" className="btn-outline btn-sm">
                    System Control
                  </a>
                  {pending.length > 0 && (
          ...
          <div className="section-wrap py-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {[
                ...
                <div key={s.label} className="stat-card">
                  <span className="font-display font-bold text-3xl text-foreground tabular-nums block mb-1">
                    {s.value}
                  </span>
                  <p className="font-body text-sm text-muted-foreground">
                    {s.label}
            ...
            <div className="flex gap-2 mb-6 p-1 bg-muted rounded-xl w-fit">
              <button
                ...
                className={
                  tab === "approvals" ? "tab-btn-active" : "tab-btn-inactive"
                }
              >
                Owners{" "}
                {pending.length > 0 && (
                  <span className="badge-danger ml-1 text-[10px]">
                    {pending.length}
                  </span>
                )}
              </button>
              <button
                onClick={() => setTab("pgs")}
                className={
                  tab === "pgs" ? "tab-btn-active" : "tab-btn-inactive"
                }
              >
                Pending PGs{" "}
              ...
              <button
                ...
                className={
                  tab === "properties" ? "tab-btn-active" : "tab-btn-inactive"
                }
              >
              ...
              <button
                onClick={() => setTab("kyc")}
                className={
                  tab === "kyc" ? "tab-btn-active" : "tab-btn-inactive"
                }
              >
                KYC Review
              </button>
              ...
              pending.length === 0 ? (
                ...
                <div className="space-y-3">
                  {pending.map((owner) => (
                    <div
                      key={owner.id}
                      className="rounded-xl bg-popover shadow-soft p-5 flex items-center gap-4"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span
                            className={`badge text-[10px] ${owner.role === "BROKER" ? "badge-pending" : "badge-default"}`}
                          >
                            {owner.role}
                          </span>
                        </div>
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button
                          onClick={() => approveOwner(owner.id, true)}
                          className="btn-primary btn-sm"
                        >
                          Approve
                        </button>
                        <button
                          onClick={() => approveOwner(owner.id, false)}
                          className="btn-outline btn-sm"
                        >
                          Reject
                        </button>
                ...
                <div className="space-y-3">
                  {properties
                    ...
                    .map((p) => (
                      <div
                        key={p.id}
                        className="rounded-xl bg-popover shadow-soft p-5 flex items-center gap-4 border-l-4 border-l-orange-400"
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
        ...
        {toast && (
          <div
            className={`fixed bottom-6 right-6 z-50 rounded-xl px-5 py-3.5 shadow-elevated font-display font-medium text-sm
