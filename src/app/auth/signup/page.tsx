...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (sr?.error) { router.push("/auth/login"); return }
    router.push(role === "TENANT" ? "/dashboard" : "/owner/dashboard")
  }

  return (
    <main><div className="min-h-screen flex bg-background">

      {/* Left panel */}
      <div className="hidden lg:flex lg:w-[52%] relative overflow-hidden">
        <Image
          src="https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=1200&q=90"
          alt="Cozy room" fill priority className="object-cover"
        />
        ...
        <div className="relative z-10 flex flex-col justify-between p-14 w-full">
          <Link href="/" className="flex items-center gap-2.5">
            <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center shadow-elevated">
              <span className="font-display font-bold text-sm text-white">PG</span>
            </div>
            <span className="font-display font-bold text-xl text-white">PGLife</span>
          </Link>
          <div>
            <h2 className="font-display font-bold text-4xl text-white mb-4 leading-tight" style={{ letterSpacing: "-0.03em" }}>
              Join 3,000+<br />happy tenants.
            </h2>
            <p className="font-body text-white/60 text-base mb-10">
              No broker fees. No fake listings. Just real homes, real owners.
            </p>
            <ul className="space-y-3">
              {["100% verified listings", "Direct owner contact via WhatsApp", "Token booking — hold your room in 2 min"].map(f => (
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-[400px]">
          ...
          <div className="flex items-center gap-3 mb-8">
            {["Choose role", "Your details"].map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                {i > 0 && <div className={`h-px flex-1 w-8 transition-colors ${step === "details" ? "bg-primary" : "bg-muted"}`} />}
                <div className={`flex items-center gap-1.5 text-xs font-display font-medium transition-colors ${
                  (i === 0 && step === "role") || (i === 1 && step === "details") ? "text-primary" : "text-muted-foreground"
                }`}>
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] transition-all ${
                    (i === 0 && step === "details") ? "bg-success text-white" :
                    (i === 0 && step === "role") || (i === 1 && step === "details") ? "bg-primary text-white" :
          ...
          {step === "role" && (
            <div>
              <h1 className="font-display font-bold text-2xl text-foreground mb-2" style={{ letterSpacing: "-0.02em" }}>
                How will you use PGLife?
              </h1>
              <p className="font-body text-sm text-muted-foreground mb-8">Choose your role to get started</p>
                ...
                {ROLES.map(r => (
                  <button key={r.value} onClick={() => setRole(r.value)}
                    ...
                    style={{
                      ...
                      background: role === r.value ? "hsl(var(--primary)/0.05)" : "hsl(var(--popover))",
                    }}
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <p className="font-display font-semibold text-base text-foreground">{r.label}</p>
                          {role === r.value && (
                        ...
                        <p className="font-body text-sm text-muted-foreground mt-0.5">{r.desc}</p>
                      </div>
                    </div>
                  </button>
                ))}
              ...
              <button onClick={() => setStep("details")} className="btn-primary w-full h-12">
                Continue →
              </button>
            </div>
          )}
          ...
          {step === "details" && (
            <div>
              <button onClick={() => setStep("role")} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-6 transition-colors cursor-pointer">
              ...
              <p className="font-body text-sm text-muted-foreground mb-8">
                Signing up as a <span className="text-primary font-medium">{role === "TENANT" ? "Tenant" : "PG Owner"}</span>
              </p>
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                {[
                  ...
                  <div key={field.key}>
                    <label className="font-display text-sm font-medium text-foreground block mb-1.5">{field.label}</label>
