...
function FilterContent({ city, onClose }: { city: string; onClose?: () => void }) {
  ...
  const reset = () => {
setGender(""); setFoodPlan(""); setMoveIn(""); setMinRent(""); setMaxRent(""); setAmenities([])
  ...
  const activeCount = [gender, foodPlan, moveIn, minRent, maxRent, ...amenities].filter(Boolean).length
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Food */}
      <div>
        <p className="font-display text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-3">Meal Plan</p>
        <div className="flex flex-wrap gap-2">
          {[
            { value: "", label: "Any" },
            ...
            <button key={f.value} onClick={() => setFoodPlan(f.value)}
              className={`font-body text-xs px-3 py-2 rounded-lg font-medium transition-all cursor-pointer ${
                foodPlan === f.value
                  ? "bg-primary text-primary-foreground shadow-soft"
                  : "bg-muted text-muted-foreground hover:bg-muted/80"
      ...
      {/* Move-in date */}
      <div>
        <p className="font-display text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-3">Move-in Date</p>
        <input
          type="date"
          value={moveIn}
