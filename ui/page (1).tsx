    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative">
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-12">
          ...
          <div className="space-y-4">
            {["Zero broker fees", "Verified listings only", "Direct owner contact", "Book in minutes"].map(f => (
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white overflow-y-auto">
        <div className="w-full max-w-sm">
          ...
          <h1 className="font-serif text-3xl font-semibold text-[#1B3B6F] mb-1">Create account</h1>
          <p className="text-gray-400 text-sm mb-8">Join thousands finding great PGs</p>
          ...
          <div className="grid grid-cols-3 gap-2 mb-5">
            {ROLES.map(r => (
              ...
              >
                ...
                <span className="text-[10px] text-center leading-tight opacity-70">{r.desc}</span>
              </button>
            ))}
          </div>
          ...
          {form.role !== "TENANT" && (
            <p className="text-xs text-amber-600 bg-amber-light border border-amber-200 rounded-xl px-4 py-3 mt-4">
              ⏳ Owner/Broker accounts require admin approval before you can list PGs.
            </p>
          )}

          <p className="text-center text-sm text-gray-400 mt-6">
            Already have an account?{" "}
            <Link href="/auth/login" className="text-[#1B3B6F] font-medium hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
