            ...
            {stats.cities.map(c => (
              <Link key={c.city} href={`/properties/${c.city.toLowerCase()}`}>
                <div className="group relative rounded-2xl overflow-hidden aspect-[3/2] cursor-pointer">
                  <img
                    src={CITY_IMAGES[c.city.toLowerCase()] ?? "https://images.unsplash.com/photo-1477959858617-67f85cf4f1df?w=400&q=80"}
                    alt={c.city}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <p className="font-serif text-white font-semibold text-base capitalize">{c.city}</p>
                    <p className="text-white/60 text-xs">{c._count}+ PGs</p>
                  </div>
                </div>
              </Link>
            ))}
